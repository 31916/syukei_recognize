...
被験者 ID と手形番号に `_` は使えません。卒業論文 表1に基づく[全63ラベルの一覧と59手形への採用区分](labels.md)を掲載しています。
...
角度に使う3点の組は `extract_hand_features.py` の `ANGLE_LANDMARK_TRIPLETS` に定義しています。`yaw` は3次元姿勢や手の表裏を表す値ではありません。学習に使うのは `angles` のみです。資料に記載された入力特徴量・単位との違いは[出典と相違点](sources.md#implementation-differences)に整理しています。
